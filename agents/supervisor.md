...
- UI層（React）/ IPC層（Tauri invoke）/ 比較エンジン（Rust）の責務を分離する
- 比較結果はモードによらず `FileNode[]` に正規化し、表示コンポーネントを共通化する
- git 操作は Rust 側でシステム `git` CLI を実行する（読み取り専用コマンドのみ、SPEC.md §6.3）
- UIはモックデータで先行開発し、Tauri IPC は後続 issue で差し替える
- MVP外の機能は含めない
...
1. SPEC.md を読む
2. issue を作成する（5〜8個）
3. issues/ に保存する（issues/issue-template.md に従う）
4. 依存関係を整理し、docs/workflow.md を更新する
5. 実行順を決定する
